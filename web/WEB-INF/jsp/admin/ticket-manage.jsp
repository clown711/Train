...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 车票管理 <span
        class="c-gray en">&gt;</span> 车票列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <div class="dataTables_wrapper no-footer">
            ...
            method-delete="ticket_delete" method-edit="ticket_edit" id="EP_table" role="grid" url="<%=basePath%>/admin/adminTicketSelect" current_page="1">
                <thead>
					<tr class="text-c">
						<th field="ticketId" width="40">ID</th>
						<th field="tripsName" width="150">所属车次</th>
						<th field="ticketSeattype" width="100">类型</th>
						<th field="ticketPrice" width="100">票价</th>
						<th field="ticketNumber" width="100">数量</th>
						<th field="EP_opation" button-type="1_2" width="100">操作</th>
					</tr>
                </thead>
...
function ticket_delete(dom) {
	var ticketId = $(dom).parent().parent().children("[name='ticketId']").text();
	var layerId = layer.confirm('确认删除', {
	  btn: ['是','否']
	}, function(){ 
		 layer.close(layerId);
		 $.ajax({
		        url:"<%=basePath%>admin/adminTicketDelete",
		        type: "POST",
		        dataType: "json",
		        async: false, 
		        data: {
...
function ticket_edit(dom){
	var ticketId = $(dom).parent().parent().children("[name='ticketId']").text();
	var index = layer.open({
		type: 2,
		title:'编辑车票信息',
		content: "<%=basePath%>admin/toAdminTicketEdit?ticketId="+ticketId,
		end:function(){
			getData();
		}
	});
