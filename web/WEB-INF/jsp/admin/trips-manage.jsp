...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 车次管理 <span
        class="c-gray en">&gt;</span> 车次列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <div class="dataTables_wrapper no-footer">
            ...
            <table class="table table-border table-bordered table-hover table-bg table-sort dataTable no-footer" 
              method-delete="trips_delete"  method-edit="trips_edit" id="EP_table" role="grid" url="<%=basePath%>/admin/adminTripsSelect" current_page="1">
                <thead>
					<tr class="text-c">
						<th field="tripsId" width="40">ID</th>
						<th field="tripsName" width="150">车次编号</th>
						<th field="tripsOrigin" width="100">始发地</th>
						<th field="tripsDestination" width="100">目的地</th>
						<th field="tripsBegintime" width="100">出发时间</th>
						<th field="tripsEndtime" width="100">到达时间</th>
...
function trips_delete(dom) {
	var tripsId = $(dom).parent().parent().children("[name='tripsId']").text();
	var layerId = layer.confirm('确认删除', {
	  btn: ['是','否']
	}, function(){ 
		 layer.close(layerId);
		 $.ajax({
		        url:"<%=basePath%>admin/adminTripsDelete",
		        type: "POST",
		        dataType: "json",
		        async: false, 
		        data: {
...
function trips_edit(dom){
	var tripsId = $(dom).parent().parent().children("[name='tripsId']").text();
	var index = layer.open({
		type: 2,
		title:'编辑车次信息',
		content: "<%=basePath%>admin/toAdminTripsEdit?tripsId="+tripsId,
		end:function(){
			getData();
		}
	});
